...
```bash
gary-booking-sam-app$ sam build
```
...
```bash
gary-booking-sam-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
gary-booking-sam-app$ sam local start-api
gary-booking-sam-app$ curl http://localhost:3000/
```
...
```bash
gary-booking-sam-app$ cd hello-world
...
```bash
sam delete --stack-name gary-booking-sam-app
```
